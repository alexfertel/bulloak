...
fn assert_on_all_parsers(
    ...
    let tree_path = tests_path.join(treefile);
...
fn scaffold_invalid_glob_warns_but_no_output() {
    ...
    assert!(
        out.status.success(),
        "scaffold should succeed even on invalid glob, got {out:?}"
    );

    let stderr = String::from_utf8_lossy(&out.stderr);
    assert!(
        stderr.contains("could not expand"),
        "did not see the expected warn: {stderr}"
    );

    let stdout = String::from_utf8_lossy(&out.stdout);
    assert!(
        !stdout.contains("contract "),
        "unexpected scaffold output: {stdout}"
    );
}
