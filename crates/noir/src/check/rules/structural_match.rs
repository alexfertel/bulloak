...
mod check_test {
    ...
    fn test_check_passes_when_correct() {
        ...
        let file_stem =
            tree_file.path().file_stem().unwrap().to_str().unwrap().to_string();

        // can't easily create a temp filename with a constant name, so we edit the content instead
        ...
        let test_path =
            tree_file.path().with_file_name(format!("{}_test.nr", file_stem));
